...
int main(int argc, char* argv[]){
	...
	if(fp){
		...
		//please enter y or n, not more than 1000 characters...
		char* c = (char*)malloc(sizeof(char) * 1000); //freed
		int again = 0;
		do{
			if(again){
				printf("Type 'y' or 'n'\n");
	...
	//time to write everything to output file
	...
	while(i < 1000){
		if(hashTable[i] != NULL){
			...
			//corner cas for debugging, should not occur
			if(temp == NULL){
				break;
			}
	...
	if(!(dirp = opendir(path))){ //dirp cannot be opened, path is file or not valid name
		...
		while((dp = readdir(dirp)) != NULL){
			...
			if(dp -> d_type == DT_REG){ //is regular file
				...
				printf("warning: your directory structure contains something that is not a text file or a directory\n");
			}
		}
	}
	...
	while(head != NULL){
		...
		//word exists as a kw
			...
			if(recordList == NULL){ //filename doesn't exist in record list
				...
				//free newNode because it's not needed
				free(newNode);
			}
